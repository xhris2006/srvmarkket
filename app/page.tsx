...
const CATEGORIES = CATEGORY_OPTIONS.slice(0, 8).map((category, index) => ({
  ...category,
  color: [
    ...
    <div className="min-h-screen bg-white">
      <section className="relative overflow-hidden bg-gradient-to-br from-purple-900 via-purple-700 to-violet-600 text-white">
        ...
        <nav className="relative z-10 flex items-center justify-between px-6 py-5 max-w-6xl mx-auto">
          ...
          <div className="flex items-center gap-3">
            {user ? (
              <a href="/search" className="bg-white text-purple-700 px-4 py-2 rounded-xl text-sm font-semibold hover:bg-purple-50 transition-colors">
                Browse Services
              </a>
            ) : (
              <>
      ...
      <section className="px-6 py-16 max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-4 md:grid-cols-8 gap-4">
          {CATEGORIES.map((cat) => (
            ...
            >
              <div className={`w-14 h-14 md:w-16 md:h-16 rounded-2xl bg-gradient-to-br ${cat.color} flex items-center justify-center shadow-sm group-hover:scale-105 transition-transform`}>
      ...
      <section className="px-6 py-20 max-w-4xl mx-auto text-center">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          {user ? (
            <>
              <a href="/search" className="gradient-brand text-white px-8 py-4 rounded-2xl font-semibold hover:opacity-90 transition-opacity">
                Browse Services
              </a>
              <a href="/account" className="border-2 border-purple-200 text-purple-700 px-8 py-4 rounded-2xl font-semibold hover:bg-purple-50 transition-colors">
                Go to Dashboard
              </a>
            </>
          ) : (
